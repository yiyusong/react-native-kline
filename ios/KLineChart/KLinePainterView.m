...
-(void)drawLongPressCrossLineText:(CGContextRef)context curPoint:(KLineModel *)curPoint curX:(CGFloat)curX y:(CGFloat)y {
    ...
    if(curX > self.frame.size.width / 2) {
        ...
        [self.mainRenderer drawText:text atPoint:CGPointMake(self.frame.size.width - textWdith - 2, y - rect.size.height / 2) fontSize:ChartStyle_defaultTextSize textColor: [UIColor, ChartColors_realTimeTextColor.CGColor]];
    } else {
        isLeft = false;
        ...
        CGContextSetFillColorWithColor(context, ChartColors_markerBgColor.CGColor);
        CGContextDrawPath(context, kCGPathFillStroke);
        [self.mainRenderer drawText:text atPoint:CGPointMake(2, y - rect.size.height / 2) fontSize:ChartStyle_defaultTextSize textColor: [UIColor, ChartColors_realTimeTextColor.CGColor]];
    }
